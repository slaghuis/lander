...
//  : Node("lander", options)
  ...
  void TargetLanderNode::init() {
    ...
    echoListener_ = std::make_shared<echoListener>(clock);
    ...
    while (rclcpp::ok() && !echoListener_->buffer_.canTransform(
  ...
  void TargetLanderNode::odometry_callback(const nav_msgs::msg::Odometry::SharedPtr msg) const
  {
     ...
     try {
       ...
       echoListener_->buffer_.transform(odom_pose, base_link_pose, "base_link", tf2::durationFromSec(0.1));
       ...
       RCLCPP_ERROR(this->get_logger(), "The current list of frames is: %s", echoListener_->buffer_.allFramesAsString().c_str() ); 
