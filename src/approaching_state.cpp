...
/* *****************************************************************************
 *  Position the drone over the landing target (fine tuning) whilst correcting
 *    the yaw to the requested landing position. Maintain altitude.
 ...
 *  Mission is complete when the position and velcity errors are within
 *    tollerance.
 * ****************************************************************************/
#include "lander/state.hpp"
#include "lander/lander_node.h"
  
void ApproachingState::execute_mission() 
{
  RCLCPP_INFO(node_->get_logger(), "Approach: Center over the goal.");
  ...
  node_->get_parameter("descend_holddown", holddown_); 
  ...
  descend_holddown_timer = std::make_shared<HolddownTimer>(holddown_);
  
  // Correct the landing approach. If the target is lost, return to seeking state and try again.
  if (correct_yaw() ) {
    node_->TransitionTo(new DescendingState);
  } else {
    node_->TransitionTo(new SeekingState);
  }
}

void ApproachingState::abort()
{
  // Stabilise flight
  node_->stop_movement();   
...
bool ApproachingState::correct_yaw() {  
  ...
  do {
    ...
    z_error = (z_error - target_seek_altitude_);
    altitude_is_close = fabs(z_error) < altitude_threshold_;
    
    if ( descend_holddown_timer->test(pose_is_close && position_is_close && altitude_is_close) ) {
      return true;
    }
